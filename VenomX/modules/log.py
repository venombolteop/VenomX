...
from VenomX.helpers.SQL import no_log_pms_sql
...
async def set_log_p_m(client: Client, message: Message):
    if BOTLOG_CHATID != -100:
        if no_log_pms_sql.is_approved(message.chat.id):
            ...
            await message.edit("**LOG Chat from this Group has been successfully activated**")
...
async def set_no_log_p_m(client: Client, message: Message):
    if BOTLOG_CHATID != -100:
        if not no_log_pms_sql.is_approved(message.chat.id):
            ...
            await message.edit("**LOG Chat from this group has been successfully disabled**")
...
async def set_pmlog(client: Client, message: Message):
    if BOTLOG_CHATID == -100:
        return await message.edit(
            "**To Use this Module, You Must Set** `BOTLOG_CHATID` **in Config Vars**"
        )
    ...
    if PMLOG:
        ...
        await edit_or_reply(message, "**PM LOG is Turned Off**")
...
async def set_gruplog(client: Client, message: Message):
    ...
    if GRUPLOG:
        ...
        await edit_or_reply(message, "**Group Log Is Off**")
...
add_command_help(
     ...
     [
         [
             "log",
             "To enable Chat Logs from that chat/group.",
         ],
         [
             "nolog",
