...
from VenomX.utils.misc import extract_args
...
async def delayspam(client: Client, message: Message):
    if message.chat.id in BLACKLIST_CHAT:
        return await edit_or_reply(
            message, "**This command is restricted for use in this group**"
        )
...
async def sspam(client: Client, message: Message):
    ...
    cooldown = {"spam": 0.15, "startspam": 0.1, "slowspam": 0.9, "fastspam": 0}
    ...
    for msg in range(amount):
        ...
        if message.command[0] == "startspam":
...
add_command_help(
     "spam",
     [
         ["spam <spam count> <text>", "Spamming text in chat!!"],
         [
             "delayspam <seconds> <number of spam> <text>",
