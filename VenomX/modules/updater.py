...
from VenomX.helpers.misc import HAPP, XCB
from VenomX.helpers.tools import get_arg
from VenomX.utils.misc import restart
from VenomX.utils.pastebin import PasteBin
from VenomX.utils.tools import bash
...
async def upstream(client: Client, message: Message):
    ...
    if "deploy" not in conf:
        if changelog:
            ...
            if len(changelog_str) > 4096:
                await status.edit("**Changelog is too large, sent as a file.**")
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Update Deploy VenomX-Userbot In Process...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`VenomX-Userbot Updated Successfully! Userbot can be used again.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
add_command_help(
     "update",
     [
         ["update", "To see a list of the latest updates from VenomX-Userbot."],
