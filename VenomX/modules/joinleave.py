...
async def join(client: Client, message: Message):
    ...
    try:
        await xxnx.edit(f"**Successfully Joined Chat ID** `{Man}`")
...
async def leave(client: Client, message: Message):
    ...
    if message.chat.id in BLACKLIST_CHAT:
        return await xxnx.edit("**This command is prohibited for use in this group**")
    try:
    ...
    async for dialog in client.get_dialogs():
        if dialog.chat.type in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
            ...
            try:
                ...
                er += 1
    await Man.edit(
        f"**Successfully Leave {done} Group, Failed to Leave {er} Group**"
    )
    ...
    async for dialog in client.get_dialogs():
        if dialog.chat.type in (enums.ChatType.CHANNEL):
            ...
            try:
                ...
                er += 1
    await Man.edit(
        f"**Successfully Exited {done} Channel, Failed to Exit {er} Channel**"
    )
...
add_command_help(
     ...
     [
         [
             "kickme",
             "Leave the group by displaying the message has left this group, bye!!.",
         ],
         ["leaveallgc", "Leave all telegram groups you joined."],
         ["leaveallch", "Leave all telegram channels you joined."],
         ["join <UsernameGC>", "To Join Chat Via username."],
