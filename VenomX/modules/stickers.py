...
from VenomX.helpers.tools import get_arg, get_text, resize_media
from VenomX.utils.tools import add_text_img, bash

from .help import add_command_help


@Client.on_message(filters.command(["kng","kang"], cmd) & filters.me)
async def kang(client: Client, message: Message):
    user = client.me
    replied = message.reply_to_message
    Man = await edit_or_reply(message, "`You can also leave the sticker out...`")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await Man.edit("**Stickers don't have Names!**")
                return
            ...
            await Man.edit("**File Not Supported**")
            return
        media_ = await client.download_media(replied, file_name="VenomX/resources/")
    else:
        await Man.edit("**Please Reply to Media Photo/GIF/Sticker!**")
        return
    if media_:
        ...
        if exist is not False:
            ...
            await Man.edit("`Create a New Sticker Pack`")
            try:
                await client.send_message("Stickers", cmd)
            except YouBlockedUser:
                await client.unblock_user("stickers")
...
async def cb_sticker(client: Client, message: Message):
    ...
    if not query:
        return await edit_or_reply(message, "**Enter the Sticker Pack Name!**")
...
async def tinying(client: Client, message: Message):
    ...
    if not (reply and (reply.media)):
        return await edit_or_reply(message, "**Please reply to sticker messages!**")
    Man = await edit_or_reply(message, "`Processing . . .`")
    ik = await client.download_media(reply)
    im1 = Image.open("VenomX/resources/blank.png")
    if ik.endswith(".tgs"):
...
async def memify(client: Client, message: Message):
    if not message.reply_to_message_id:
        await edit_or_reply(message, "**Reply to photo or sticker messages!**")
        return
    reply_message = message.reply_to_message
    if not reply_message.media:
        await edit_or_reply(message, "**Please reply to photos or stickers!**")
        return
    file = await client.download_media(reply_message)
    Man = await edit_or_reply(message, "`Processing . . .`")
    text = get_arg(message)
    if len(text) < 1:
        return await msg.edit(f"Please Type `{cmd}mmf text`")
...
add_command_help(
     ...
     [
         [
             f"kang `or` {cmd}ticle",
             f"Reply {cmd}kang To Sticker Or Image To Add To Sticker Pack.",
         ],
         [
             f"kang [emoji] `or` {cmd}ticle [emoji]",
             f"To add and customize emoji stickers to your sticker pack.\n\n` • **NOTE:** To create a new sticker pack, use the number after {cmd}kang\n • **EXAMPLE:** {cmd}kang 2 to create and save to the 2nd sticker pack`",
         ],
         [
             f"packinfo `or` {cmd}stickerinfo",
             "To Get Sticker Pack Information.",
         ],
         ["get", "Reply to sticker to get sticker photo."],
         ["stickers <sticker name>", "To Find a Sticker Pack."],
     ],
)
...
add_command_help(
     ...
     [
         [
             ...
             "Reply to message, sticker or photo will be converted into the specified meme text sticker.",
         ],
     ],
)
